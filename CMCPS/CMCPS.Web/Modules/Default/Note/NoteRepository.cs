    ...
    public class NoteRepository
    {
        ...
        private class MySaveHandler : SaveRequestHandler<MyRow> {
            protected override void BeforeSave()
            {
                ...
                if (IsCreate)
                {
                    if (!this.Connection.Exists<NoteRow>(MyRow.Fields.AdmissionId == Convert.ToString(Request.Entity.AdmissionId.Value)))
                    {
                        // do anything you want to do
                    }
                    else
                    {
                        throw new ValidationError("This patient for note already saved.");
                    }
                }
